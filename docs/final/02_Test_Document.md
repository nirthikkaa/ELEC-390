...
| Status | Count |
|---|---|
| Total rows in this submission draft | `51` |
| `Pass` | `17` |
...
- `Pass` means the row is already backed by the current source and recorded automated evidence.
- `Pending manual verification` means the feature exists, but the final submission still needs an explicit physical Pixel 7 check or timed demo-device check.
- This table is meant to be honest final-submission evidence, not a one-row-per-test dump.
